...
- **Quality Hook:** Checks now run only against files the project owns, rooted at the checkout that owns them. A file outside `CLAUDE_PROJECT_DIR` (a sibling repo, a `/tmp` scratchpad) is skipped instead of blocked by an unrelated project's formatter and lint rules, via a new `kind === "outside"` containment gate in `resolveFileRoot`. Files in a linked git worktree now run from the worktree root, with the correct `cwd`, the correct `node_modules`, and a worktree-relative `$FILE` in `<reproduce-with>`. Agent delegation (PreToolUse) applies the same rule and no longer fires on files the project does not own. ([#8](https://github.com/rcrsr/checkmate/pull/8))
- **Quality Hook:** `resolveFileRoot` now returns its realpath'd `filePath` alongside `root`, and every caller (`getChecksForFile`, `resolveCommand`, `getAgentForFile`) relativizes against that value instead of the raw `tool_input` path. Previously a symlinked ancestor (a symlinked checkout, a bind mount) put `root` and `filePath` in 2 different coordinate spaces, so `path.relative()` returned a `../`-prefixed path: checks silently skipped, `<reproduce-with>` emitted an unusable command, and pre-tool delegation silently stopped blocking main-thread edits.
- **Exclude Patterns:** `matchesExcludePattern()` now escapes regex metacharacters, including `?`. `.` previously acted as a wildcard, so `"../**"` compiled to `/^..\/.*$/` and silently excluded any two-character top-level directory (`ui/`, `db/`, `js/`); `?` previously behaved as a regex quantifier instead of a literal character in patterns that used it. ([#8](https://github.com/rcrsr/checkmate/pull/8))
- **Path Matching:** `fileMatchesPaths()` and `matchesExcludePattern()` now normalize both the computed relative path and the configured `paths`/`exclude` entries to POSIX-style (`/`) separators before comparing. `path.relative()` yields `\` separators on Windows, which previously matched no `paths` entry, silently disabling checks and agent delegation there.
- **Worktree Detection:** `classifyGitMarker()` no longer relies solely on the `.git/worktrees/` substring appearing in the gitdir line. A worktree created against a relocated `GIT_DIR`/`GIT_COMMON_DIR`, or a bare-repo-adjacent layout, previously fell through to `nested-repo` and silently skipped checks instead of inheriting the session config. Detection now falls back to a layout-independent, pure-fs check: a real worktree's gitdir target directory contains a `commondir` file.

### Changed

- **Submodules:** A submodule or nested repo's `.git` now only marks a project boundary; it grants no config by itself. A submodule/nested repo with its own `.claude/checkmate.json` runs that config instead of the parent's. One without its own config is skipped, since the parent's policy has no business reaching into a different project. Previously the parent's config ran against submodule files from the parent's root, resolving `node_modules` and tool config against the wrong checkout. A submodule that relied on the parent's config runs no checks until it gets one. ([#8](https://github.com/rcrsr/checkmate/pull/8))
